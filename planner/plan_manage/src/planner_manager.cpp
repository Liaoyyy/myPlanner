...
namespace ego_planner
{
                                         ...
                                         const Eigen::Vector3d &end_pos, const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc)
  {
    ...
    // generate global reference trajectory
    vector<Eigen::Vector3d> points;
    points.push_back(start_pos);//初始点
    points.push_back(end_pos);  //目标点

    // insert intermediate points if too far
    // 如果目标点与初始点距离过远（大于阈值dist_thresh）时，插入点方便minimum_snap生成轨迹
    vector<Eigen::Vector3d> inter_points;
    const double dist_thresh = 4.0; //阈值4m

    for (size_t i = 0; i < points.size() - 1; ++i)
    {
      inter_points.push_back(points.at(i));
      double dist = (points.at(i + 1) - points.at(i)).norm();//计算欧氏距离
    ...
    for (int i = 0; i < pt_num - 1; ++i)
    {
      //计算两个相邻点间运动时间，速度以最大设定速度max_vel_计算
      time(i) = (pos.col(i + 1) - pos.col(i)).norm() / (pp_.max_vel_);
    }
    ...
    //开启minimum snap生成多项式轨迹
    PolynomialTraj gl_traj;
    //当总点数大于3时，进行分段轨迹生成
    if (pos.cols() >= 3)
      gl_traj = PolynomialTraj::minSnapTraj(pos, start_vel, end_vel, start_acc, end_acc, time);
    //当总点个数等于2时，直接一段距离进行轨迹生成
    else if (pos.cols() == 2)
