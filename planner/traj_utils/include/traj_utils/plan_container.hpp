...
namespace ego_planner
{
  ...
  public:
    ...
    //存储全局轨迹，即初始minimum snap生成的轨迹
    void setGlobalTraj(const PolynomialTraj &traj, const ros::Time &time)
    {
      global_traj_ = traj;
      global_traj_.init();//计算按当前轨迹traj运动总时间
      global_duration_ = global_traj_.getTimeSum();//保存总时间
      global_start_time_ = time;//当前时间
